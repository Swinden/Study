...
# i：正在处理的是原始矩阵的哪一列
    if type == 1:  #极小型
        ...
        print("第",i,"列是区间型")
        a,b = [int(l) for l in input("按顺序输入最佳区间的左右界，并用逗号隔开：\n").split(",")]
...
if judge == 1:
    position = np.array([int(i) for i in input("请输入需要正向化处理的指标所在的列，例如第1、3、4列需要处理，则输入1,3,4：\n").split(',')])
    position = position-1
    typ = np.array([int(j) for j in input("请按照顺序输入这些列的指标类型（1：极小型，2：中间型，3：区间型）格式同上：\n").split(',')])
...
sorted_S = sorted(std_S,reverse=True)
print(std_S)  # 打印标准化后的得分
print(sorted_S)
print(np.argsort(-std_S)+1)
np.savetxt('std_S.csv',std_S,delimiter=',')  # 结果输出到std_S.csv文件
