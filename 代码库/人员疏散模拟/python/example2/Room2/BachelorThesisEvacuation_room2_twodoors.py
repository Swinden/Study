...
# data_matrix = np.loadtxt(r'room2\room2_data\room2_twodoors665', dtype=float)
...
class Agent(object):
    ...
    def update_dest(self):
        if self.pos[0] < 400:
            if self.pos[1] <= 400 and self.pos[0] > 135 :
                self.dest = np.array([135, 355])
            elif self.pos[1] > 400 and self.pos[0] > 135:
                self.dest = np.array([135, 445])
            elif self.pos[0] < 140:
                self.dest = np.array([100,400])
        else:
            if self.pos[1] <= 400 and self.pos[0] < 665:
                self.dest = np.array([665, 355])
            elif self.pos[1] > 400 and self.pos[0] < 665:
                self.dest = np.array([665, 445])
            elif self.pos[0] > 660:
                self.dest = np.array([700,400])
...
def main():
    ...
    [665,375,665,425], [665,375,680,400], [680,400,665,425], # additional walls
    [135, 375, 135, 425], [135, 375, 120, 400], [135, 425, 120, 400],  # additional walls
    ...
    pygame.quit()
main()
np.savetxt(r'room2\room2_data\room2_twodoors665', data_matrix)