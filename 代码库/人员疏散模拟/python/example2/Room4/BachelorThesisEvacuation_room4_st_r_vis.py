...
# data_matrix = np.loadtxt(r'room4\room4_data\room4_vis665', dtype=float)
...
class Agent(object):
    ...
    def update_dest(self):
        ...
        if dist < 300:
            ...
            if self.pos[1] < 410 + self.radius and self.pos[1]> 400:
                self.dest = np.array([700,400])
            elif  self.pos[1] < 400 and self.pos[1] > 390 - self.radius:
                self.dest = np.array([700,400])
            elif self.pos[1] > 690 - self.radius: 
...
def main():
    ...
    ,[100,400,665,400], # additional walls
...
main()
np.savetxt(r'room4\room4_data\room4_vis665', data_matrix)