...
walls = [[room_left, room_top, \
    room_left + room_width, room_top],
        ...
        [655, 375, 655, 425], \
            [655, 375, 670, 400], [670, 400, 655, 425],  # additional walls
         [140, 375, 140, 425], \
            [140, 375, 125, 400], [140, 425, 125, 400],  # additional walls
    [room_left, room_top+room_height, room_left + \
        room_width, room_top + room_height],
    [room_left + room_width, room_top, \
...
for j in range(0, nr_experiments):
    ...
    for i in range(0, nr_agents):  # For all objects
        ...
        while found == False:
            ...
            for wall in walls:
                r_i = radius
                d_iw, e_iw = distance_agent_to_wall(
                    np.array([object_x, object_y]), wall)
                if d_iw < r_i:
                    countwall += 1
            ...
            if len([positionmatrix[i] for i in range(j*nr_agents, j*nr_agents + agents_found)]) > 0:
                ...
                for position in [positionmatrix[i] for i in range(j*nr_agents, j*nr_agents + agents_found)]:
                    dist = math.sqrt(
                        (position[0]-object_x)**2 + (position[1]-object_y)**2)
        ...
        positionmatrix.append(
            [object_x, object_y, radius, mass, desiredS, nr_experiment])
