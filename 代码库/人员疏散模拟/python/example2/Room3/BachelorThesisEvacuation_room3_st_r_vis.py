...
# data_matrix = np.loadtxt(r'room3\room3_data\room3_vis367', dtype=float) # Enable after first run
...
class Agent(object):
    ...
    def update_dest(self):
        ...
        if dist < 300:
            if self.pos[0] > 655 and self.pos[1] > 433:
                self.dest = np.array([645,400])
            elif self.pos[0] > 655 and self.pos[1] < 367:
                self.dest = np.array([645,400])
            else:
...
def main():
    ...
    [room_left, room_top+room_height, room_left, room_top], [675, 367, 725, 367], [675,367, 700, 382],
    ...
    pygame.quit()
main()
np.savetxt(r'room3\room3_data\room3_vis367', data_matrix)