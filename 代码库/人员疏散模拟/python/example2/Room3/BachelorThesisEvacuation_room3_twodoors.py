...
# data_matrix = np.loadtxt(r'room3\room3_data\room3_twodoors367', dtype=float)
...
class Agent(object):
    ...
    def update_dest(self):
        if self.pos[0] > 400:
            if self.pos[0] > 655 and self.pos[1] > 439:
                self.dest = np.array([650,400])
            elif self.pos[0] > 655 and self.pos[1] < 361:
                self.dest = np.array([650,400])
            else:
                self.dest = np.array([700,400])
        else:
            if self.pos[0] < 155 and self.pos[1] > 439:
                self.dest = np.array([160,400])
            elif self.pos[0] < 155 and self.pos[1] < 361:
                self.dest = np.array([160,400])
            else:
...
def main():
    ...
    [725,367, 700, 382], [675,433, 700, 418], [725,433, 700, 418], [675, 433, 725, 433], 
    [125,367, 100, 382], [75,433, 100, 418], [125,433, 100, 418], [75, 433, 125, 433], 
    [675, 367, 725, 367], [675,367, 700, 382], [75, 367, 125, 367], [75,367, 100, 382],
    ...
    pygame.quit()
main()
np.savetxt(r'room3\room3_data\room3_twodoors367', data_matrix)